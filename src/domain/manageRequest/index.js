  ...
  try {
    ...
    if (mode === 'query') {
      ...
      if (typeof params !== 'object') {
        ...
        url += Object.values(params).map((v) => '/' + encodeURIComponent(v)).join(''); 
      }
    } else if (
    ...
    console.log(
    `curl -X ${method} ${url} -H "Authorization: ${token}" -H "Tokenid: ${token}" -H "Content-Type: application/json" -d '${JSON.stringify(
      params,
    )}'`,
  );
